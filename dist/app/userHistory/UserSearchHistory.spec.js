...
// src/app/userHistory/UserSearchHistory.spec.ts
var repositoryMock = {
  save: vi.fn(),
  find: vi.fn()
};
var sut = new UserSearchHistoryService();
describe("UserSearchHistoryService", () => {
  describe("addSearchHistory()", () => {
    it("Should be able to add a search history", () => __async(exports, null, function* () {
      const userIdMock = "1";
      ...
      const result = yield sut.addSearchHistory(userIdMock, searchQueryMock);
      globalExpect(result).toStrictEqual(expectMock);
    }));
  });
  describe("getLastSearches()", () => {
    it("Should be able to get the last searches", () => __async(exports, null, function* () {
      const userIdMock = "1";
      const limitMock = 5;
      const expectMock = [
      ...
      const result = yield sut.getLastSearches(userIdMock, limitMock);
      globalExpect(result).toStrictEqual(expectMock);
    }));
  });
});
