...
class TechSearchController {
  ...
  async getTopTechnologies(req: Request, res: Response) {
    try {
      const limit = 5;
      const topTechnologies = await this.techSearchService.getTopTechnologies(
        limit
      );
  ...
  async getTopCitiesForMostSearchedTech(req: Request, res: Response) {
    try {
      const topCities =
        await this.techSearchService.getTopCitiesForMostSearchedTech();
  ...
  async searchTechAndCity(req: Request, res: Response) {
    try {
      ...
      if (!technology || !city) {
        CommonError.build(
          "Technology and city Not Found",
          STATUS_CODE.NOT_FOUND
        );
      }

      const count = await this.techSearchService.searchTechAndCity(
        technology,
        city
      );
  ...
  async getTechSearchResults(req: Request, res: Response) {
    try {
      ...
      const results = await this.techSearchService.searchTech(
        req.query,
        startIndex,
        perPage
      );
